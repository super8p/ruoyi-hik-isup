...
public class StartedUpRunner implements ApplicationRunner, DisposableBean {
    ...
    private int SsHandle = -1;
    private int AlarmHandle = -1;
    private int lListenHandle = -1;
    private int VoicelServHandle = -1;
    private int m_lPlayBackListenHandle = -1;
...
//        pSSListenParam.fnSSRWCbEx = fEHomeSSRWCallBackEx;
        ...
        this.SsHandle = hikISUPStorage.NET_ESS_StartListen(pSSListenParam);
        if (this.SsHandle == -1) {
        ...
        //启动报警服务器监听
        this.AlarmHandle = hikISUPAlarm.NET_EALARM_StartListen(net_ehome_alarm_listen_param);
        System.out.println("AlarmHandle: " + this.AlarmHandle);
        ...
        this.lListenHandle = hikISUPStream.NET_ESTREAM_StartListenPreview(netEhomeListenPreviewCfg);
        log.info("lListenHandle: {}", this.lListenHandle);
        if (this.lListenHandle == -1) {
        ...
        this.VoicelServHandle = hikISUPStream.NET_ESTREAM_StartListenVoiceTalk(net_ehome_listen_voicetalk_cfg);
        if (this.VoicelServHandle == -1) {
        ...
        this.m_lPlayBackListenHandle = hikISUPStream.NET_ESTREAM_StartListenPlayBack(struPlayBackListen);
        if (this.m_lPlayBackListenHandle < -1) {
        ...
        //启动监听，接收设备注册信息
        this.CmsHandle = hcisupcms.NET_ECMS_StartListen(struCMSListenPara);
        if (this.CmsHandle < 0) {
    ...
    public void destroy() {
        log.info("========================= 关闭SDK监听服务 =========================");
        if (SsHandle != -1) {
            hikISUPStorage.NET_ESS_StopListen(SsHandle);
            SsHandle = -1;
        }
        if (AlarmHandle != -1) {
            hikISUPAlarm.NET_EALARM_StopListen(AlarmHandle);
            AlarmHandle = -1;
        }
        if (lListenHandle != -1) {
            hikISUPStream.NET_ESTREAM_StopListenPreview(lListenHandle);
            lListenHandle = -1;
        }
        if (VoicelServHandle != -1) {
            hikISUPStream.NET_ESTREAM_StopListenVoiceTalk(VoicelServHandle);
            VoicelServHandle = -1;
        }
        if (m_lPlayBackListenHandle != -1) {
            hikISUPStream.NET_ESTREAM_StopListenPlayBack(m_lPlayBackListenHandle);
